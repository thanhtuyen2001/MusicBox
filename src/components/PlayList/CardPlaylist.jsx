...
const CardPlaylist = ({item}) => (
    <div className=" p-4 w-52 rounded-md">
     ...
     <div className="rounded-md overflow-hidden group relative ">
        <img src={item.src ? item.src : songCover} alt="playlist" className="group-hover:scale-125 group-hover:opacity-70 transition-all" />
     ...
     {/* title */}
     <div>
        <div className="text-lg font-medium mt-1 w-[90%] inline-block truncate  cursor-pointer" title={item.title}>{item.title}</div>
     </div>
     {/* desc */}
     <div>
        <div className="text-[#8792a2] text-sm  line-clamp-2 cursor-pointer" title={item.desc}>{item.desc}</div>
     </div>
